...
static inline float v3Dot(Vector3 a, Vector3 b) {
    // Effectively an cosinus value of the angle between given vectors
    //   If one of the vectors is a unit vector result is identical to
    //   projection of the other vector on this unit vector
...
static void triangulateXYPlane(Vertices3 *vertices3D, float delta) {
    ...
    for (float x = -1; x < 1; x += delta)
        for (float y = -1; y < 1; y += delta) {
            // here we have a square: (x, y) -> (x + dt, y) -> (x + dt, y + dt) -> (x, y + dt)
            // this square gives us 2 triangels to add to vertices
            Vector3 a = {x,         y,      0};
...
static int cmp_triangle2_far_to_near(const void *pa, const void *pb) {
    ...
    // camparing float values, but returning int
    // so (b->depth - a->depth) would produce too many falsely equal results (rounding towards zero)
...
static void addSinVertice(Vertices2 *to, Vector3 ac, Vector3 bc, Vector3 cc) {
    ...
    if (cross < 0) {
        ...
        // swap b/c to keep consistent winding if you want
        ...
        if (stage == STAGE_GEN) {
            ...
            for (size_t n = start; n < end; n++) {
                ...
                for (size_t cnt = 0; cnt < 3; cnt++) {
                    ...
                    v->z = plot_function_star(v->x, v->y, job->plot_phase);
                    // world coordinates
                ...
                switch (clipPolyNearPlane(points_in, points_out)) {
                    case 0:
                        ...
                        continue;
                    case 3:
                        addSinVertice(vertices2D, points_out[0], points_out[1], points_out[2]);
                        break;
                    case 4:
                        addSinVertice(vertices2D, points_out[0], points_out[1], points_out[2]);
                        addSinVertice(vertices2D, points_out[2], points_out[3], points_out[0]);
                        break;
                    default:
                        printf("UNREACHABLE: drawSin3D, id: %zu\n", id);
                        abort();
                }
            }
        } else { // STAGE_ZPASS
            // Each worker owns a disjoint set of tiles -> no locks on zbuf/idbuf
...
static void drawSin3D(Vertices2 *vertices2D, Vertices3 *vertices3D, const Vector3 camera, Vector3 i, Vector3 j, Vector3 k, float plot_phase) {
...
int main(void) {
    ...
    while (!WindowShouldClose())
    {
        ...
        drawSin3D(&vertices2D, &vertices3D, camera, forward, right, up, plot_phase);
        plot_phase += 0.1;
